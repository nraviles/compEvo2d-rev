# -*- coding: utf-8 -*-
"""
Created on Fri Aug 13 08:54:04 2021
...
tau_a_i =  np.zeros(subsamp)      # rate of adaptation in absolute fitness trait alone
tau_r_i =  np.zeros(subsamp)      # rate of adaptation in relative fitness trait alone
rho_ar_i =  np.zeros(subsamp)      # rate of fitness decrease due to environmental degradation
...
for i in range(subsamp):
    ...
    tau_a_i[i] = myfun.get_tau(parValue[parName['sa']],Uai[i])
    tau_r_i[i] = myfun.get_tau(eff_sr_i[i],parValue[parName['Ur']])
    rho_ar_i[i] = tau_a_i[i]/tau_r_i[i]
...
ax1.scatter(absFitClass,tau_a_i,color="blue",linewidth=1.0,label=r'$v_a$')
ax1.scatter(absFitClass,tau_r_i,color="red",linewidth=1.0,label=r'$v_r$')
ax1.plot(absFitClass,rho_ar,i,color="green",linewidth=2,label=r' $R$')
