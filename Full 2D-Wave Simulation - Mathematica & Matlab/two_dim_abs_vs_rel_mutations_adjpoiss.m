...
% pop = Bin(pop, 1 - 1/di) should I write this as di = d(1+sa)^fit_a ?
    
% drift cutoff
stoch = (pop<cutoff);
pop(stoch) = poissrnd(pop(stoch)); %here it is, the poisson part, I think all I need to do is adjust this
% so instead I would basically steal Jasons model of selecting competition
% wins based on a random sampling of uniform. I'm uncertain regarding what
% we would do to identify the populations and all of the unoccupied
% territories; probably just strip the whole processes.
pop = round(pop);
