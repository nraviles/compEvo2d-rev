...
t = time.time()
element_run_eqpop = Parallel(n_jobs=-1)(delayed(myfun.get_eq_pop_density)(parValue[parName['b']],di[(k)*part],parValue[parName['sa']],yi_option) for k in range(2,subsamp)) #got rid of the minus 1; shifted k -> k+2
print(time.time() - t)  
element_run_eqpop = list(chain.from_iterable(element_run_eqpop))
...
for row in element_run_eqpop:
    parallel_eqpop.write(str(int(math.ceil(parValue[parName['T']]*row)))+'\n') # got rid of row[0]
parallel_eqpop.close()

parallel_effsr = open("parallel_effsr.txt","a") 
for row in element_run_eqpop: # b,di,y,cr
    parallel_effsr.write(str(myfun.get_c_selection_coefficient_OLD(parValue[parName['b']],row,parValue[parName['cr']]))+'\n')
...
t = time.time()
element_run_abs = Parallel(n_jobs=-1)(delayed(myfun.modsimpop)(d_Inc,c_Inc,samp,parValue[parName['T']],parValue[parName['cr']],parValue[parName['b']],di[k*part:(k*part + 2)],parValue[parName['do']],((di[k*part]/di[k*part + 1])-1)/(di[k*part + 1]-1),d_max,yi_option) for k in range(2,subsamp)) #got rid of the minus 1
print(time.time() - t)  
...
t = time.time()
element_run_rel = Parallel(n_jobs=-1)(delayed(myfun.modsimpop)(d_Inc,c_Inc,samp,parValue[parName['T']],parValue[parName['cr']],parValue[parName['b']],di[k*part:(k*part + 2)],parValue[parName['do']],((di[k*part]/di[k*part + 1])-1)/(di[k*part + 1]-1),d_max,yi_option) for k in range(2,subsamp)) #got rid of the minus 1
print(time.time() - t)  
